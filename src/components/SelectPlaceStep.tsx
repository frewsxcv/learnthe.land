        ...
        <Card.Body>
          <div className="d-grid gap-3" key={i}>
            <div style={{ height: '150px', display: 'flex' }}>
              <MapContainer attributionControl={false} touchZoom={false} tap={false} keyboard={false} boxZoom={false} doubleClickZoom={false} zoomControl={false} dragging={false} scrollWheelZoom={false} center={[0,0]} zoom={5} style={{ flexGrow: 1 }}>
                <TileLayer
...
const PlaceLayer = ({ place }: { place: Place }) => {
  const map = useMap();
  return (
    <GeoJSON data={place.geometry_geojson as any}
